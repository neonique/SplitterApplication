...
public class Gruppe {
  ...
  public void addNutzer(String name) {
    ...
    if (!validateName(name)) {
      throw new IllegalArgumentException(
          "Nutzername ist nicht konform mit GitHub-Namenskonvention");
    }

    Nutzer teilnehmer = new Nutzer(name);
    if (!transaktionen.isEmpty()) {
      throw new RuntimeException(
          "Nutzer koennen nach der ersten Transaktion nicht mehr zur Gruppe hinzugefuegt werden.");
    }
    if (this.isclosed()) {
      throw new RuntimeException(
          "Nutzer koennen nicht zu geschlossenen Gruppen hinzugefuegt werden");
    }
  ...
  public void addTransaktion(String sponsor, Set<String> bettler, double betrag, String grund) {
    ...
    if (!this.getTeilnehmerNamen().containsAll(bettler) || !this.getTeilnehmerNamen()
        .contains(sponsor)) {
    ...
    if (this.isclosed()) {
      throw new RuntimeException(
          "Transaktionen koennen nicht zu geschlossenen Gruppen hinzugefuegt werden");
    }

    transaktionen.add(
        new Transaktion(new Nutzer(sponsor), bettler.stream().map(b -> new Nutzer(b)).collect(
  ...
  private List<Transaktion> getTransaktionen(List<TransaktionDTO> transaktionenDtos) {
    ...
    for (TransaktionDTO dto : transaktionenDtos) {
      ...
      transaktionen.add(
          new Transaktion(dto.id(), nutzer, bettler, Money.of(dto.betrag(), "EUR"), dto.grund()));
    }
